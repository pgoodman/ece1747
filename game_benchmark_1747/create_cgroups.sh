...
#cgroup for server
sudo cgcreate -t $USERNAME:$GROUPNAME -a $USERNAME:$GROUPNAME -g cpuset:simmud-server
sudo cgset -r cpuset.cpus="0-$((SERVER_CPUS-1))" simmud-server
#cgroup for clients
sudo cgcreate -t $USERNAME:$GROUPNAME -a $USERNAME:$GROUPNAME -g cpuset:simmud-clients
sudo cgset -r cpuset.cpus="$((SERVER_CPUS))-$((SERVER_CPUS+CLIENT_CPUS-1))" simmud-clients

sudo cgcreate -t $USERNAME:$GROUPNAME -a $USERNAME:$GROUPNAME -g cpuset:simmud-lttng
