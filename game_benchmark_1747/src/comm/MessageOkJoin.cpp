...
MessageOkJoin::MessageOkJoin(int target)
    : Message(MESSAGE_SC_OK_JOIN, target) {
  name[0] = 0;
  x = y = 0;
  mapx = mapy = 0;
}

MessageOkJoin::MessageOkJoin(int target, IPaddress addr, char *name,
                             Vector2D pos, Vector2D map)
...
MessageOkJoin::MessageOkJoin(UDPpacket *p)
    : Message(MESSAGE_SC_OK_JOIN, 0) {
  if (p == NULL)
    return;
