...
MessageWithSerializator::MessageWithSerializator(int t, int target,
                                                 IPaddress adr)
    : Message(t, target, adr) {
  /* alocate space for a new packet */
  packet = SDLNet_AllocPacket((int) MAX_UDP_PACKET_SIZE);
  assert(packet);
...
MessageWithSerializator::MessageWithSerializator(UDPpacket *p)
    : Message() {
  if (p == NULL)
    return;
