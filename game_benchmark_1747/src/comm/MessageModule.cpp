...
void Channel::deinit() {
  ...
  if (pck != NULL)
    SDLNet_FreePacket(pck), pck = NULL;
}
...
Message* MessageModule::receive(int timeout, int chan_id) {
  ...
  if (thread_on)
    return chans[chan_id].m_queue[0].getMessage(timeout);

  res_cs = SDLNet_CheckSockets(chans[chan_id].sset, timeout);
  assert(res_cs != -1);
  if (!res_cs)
    return NULL;			// no_activity
...
void MessageModule::receive(int timeout,std::list<Message*> *lm, int chan_id) {
  ...
  if (thread_on) {
    chans[chan_id].m_queue[0].getMessages(timeout, lm);
    return;
  }
...
void MessageModule::run_receive() {
  ...
  while (!finished) {
    res_cs = SDLNet_CheckSockets(full_sset, (Uint32) UDP_TIMEOUT);
    assert(res_cs != -1);
    if (!res_cs)
      continue;			// no_activity

    for (i = 0; i < n_chans; i++) {
      if (!SDLNet_SocketReady(chans[i].sock))
        continue;

      m = receive_message(i);
      if (m)
        chans[i].m_queue[0].putMessage(m);
    }
  }
}
...
Message* MessageModule::receive_message(int chan_id) {
  ...
  if (chans[chan_id].pck == NULL)
    chans[chan_id].pck = SDLNet_AllocPacket((int) MAX_UDP_PACKET_SIZE);
  assert(chans[chan_id].pck);
  p = chans[chan_id].pck;

  res_ur = SDLNet_UDP_Recv(chans[chan_id].sock, p);
  if (res_ur < 0) {
    printf("[WARNING] Error receiving UDP packet");
    return NULL;
  }
  if (res_ur == 0) {
    printf("[WARNING] No UDP packet received on an active port");
    return NULL;
  }
...
void MessageModule::run_send() {
  ...
  while (!finished) {
    ...
    while (check_send_ready() == -1)
      if (SDL_CondWaitTimeout(send_cond, send_mutex, SEND_TIMEOUT) != 0)
        break;
    SDL_UnlockMutex(send_mutex);
    ...
    Message* m;
    while ((m = chans[send_chan_id].m_queue[1].getMessageAsync()) != NULL)
      send_message(m, send_chan_id);
...
void MessageModule::send_message(Message* m, int chan_id) {
  ...
  if (SDLNet_UDP_Send(chans[chan_id].sock, -1, p) == 0)
