...
void AStar::setSecondMatrix(int n2, int m2, char **second_matrix, int offx,
                            int offy) {
  this->n2 = n2;
  this->m2 = m2;
...
PointStack* AStar::findPathAStar(int startx, int starty, int stopx, int stopy,
  ...
  if (matrix == NULL || n <= 0 || m <= 0 || startx < 0 || startx >= n
      || starty < 0 || starty >= m || stopx < 0 || stopx >= n || stopy < 0
      || stopy >= m)
    return &path;
  if (matrix[stopx][stopy] != 0)
    return &path;
...
PointStack* AStar::findPathTh(int startx, int starty, int stopx, int stopy,
                              int explore_size) {
  path.clear();
