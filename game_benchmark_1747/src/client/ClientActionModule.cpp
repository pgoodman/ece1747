    ...
    if (m != NULL) /* new message */
    {
      ...
      switch (m->getType()) {
        case MESSAGE_SC_OK_JOIN:
          handle_OK_JOIN(m);
          break;
        case MESSAGE_SC_NOK_JOIN:
          handle_NOK_JOIN();
          break;
        case MESSAGE_SC_OK_LEAVE:
          handle_OK_LEAVE();
          break;
        case MESSAGE_SC_REGULAR_UPDATE:
          handle_REGULAR_UPDATE(m);
          break;
        case MESSAGE_SC_NEW_QUEST:
          handle_NEW_QUEST(m);
          break;
        case MESSAGE_SC_QUEST_OVER:
          handle_QUEST_OVER();
          break;
...
/* send a join request to the server */
{
  ...
  if (m == NULL)
    throw "Cannot send initial message";
...
/* play */
{
  ...
  if (client_data->debug_AI)
    printf("Action: %s\n", AI_actions[(int) action]);

  switch (action) {
    case ATTACK_UP:
      message_type = MESSAGE_CS_ATTACK_UP;
      break;
    case ATTACK_DOWN:
      message_type = MESSAGE_CS_ATTACK_DOWN;
      break;
    case ATTACK_RIGHT:
      message_type = MESSAGE_CS_ATTACK_RIGHT;
      break;
    case ATTACK_LEFT:
      message_type = MESSAGE_CS_ATTACK_LEFT;
      break;
    case USE:
      message_type = MESSAGE_CS_USE;
      break;
    case MOVE_UP:
      message_type = MESSAGE_CS_MOVE_UP;
      break;
    case MOVE_DOWN:
      message_type = MESSAGE_CS_MOVE_DOWN;
      break;
    case MOVE_LEFT:
      message_type = MESSAGE_CS_MOVE_LEFT;
      break;
    case MOVE_RIGHT:
      message_type = MESSAGE_CS_MOVE_RIGHT;
      break;
    default:
      return;
  };
...
/* retry sending the leave message to the server */
{
  ...
  if (m == NULL)
    throw "Cannot send exit message";
...
/* update game world */
{
  ...
  if (client_data->average_update_interval < 0)
    ...
    client_data->average_update_interval = client_data->average_update_interval
        * 0.95 + (double) client_data->last_update_interval * 0.05;
