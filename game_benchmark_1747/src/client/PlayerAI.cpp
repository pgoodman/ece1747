...
bool PlayerAI::findNearObject(int *rx, int *ry, int distance, int type,
  ...
  for (d = 1; d < distance; d++) {
    for (x = px - d; x <= px + d; x++) {
      ...
      i = x - px + distance;
      j = y - py + distance;
      if (cd->map[i][j].type == type && cd->map[i][j].life >= min_value
          && cd->map[i][j].life <= max_value) {
        *rx = x, *ry = y;
        return true;
      }
...
bool PlayerAI::checkFood(int amount) {
  ...
  if (lifeBelow(amount)
    ...
    if (debug_AI)
      printf("[AI]Want food at %d,%d (same position as player)\n", cd->xpos,
             cd->ypos);
    return true;
  }
...
bool PlayerAI::checkQuest() {
  if (questExists() && questIsOutOfReach()) {
    path = pathfind_alg.findPathNear(cd->xpos, cd->ypos, cd->questx,
                                     cd->questy);
    if (path->empty()) {
      if (debug_AI)
        printf("[AI]Cannot find path to quest (%d,%d -> %d,%d)\n", cd->xpos,
               cd->ypos, cd->questx, cd->questy);
      return false;
    }
...
bool PlayerAI::checkWeakPlayer() {
  ...
  if (weakPlayerNear(&x, &y)) {
    ...
    if (debug_AI)
      printf("[AI]Chasing player at %d,%d (%d moves)\n", x, y, path->size());
    return true;
  }
  return false;
}
