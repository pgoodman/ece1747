...
ClientRenderModule::ClientRenderModule(ClientData *cd) {
  ...
  if (client_data->has_GUI) {
    ...
    if (md1 == NULL || md2 == NULL)
      throw "Not enough memory to start the client";
  } else
    md1 = md2 = NULL;
  nmd = nmd2 = 0;
}
...
void ClientRenderModule::initGraphics() {
  ...
  if (icon != NULL) {
    SDL_SetColorKey(icon, SDL_RLEACCEL | SDL_SRCCOLORKEY,
                    SDL_MapRGB(icon->format, 255, 255, 255));
    SDL_WM_SetIcon(icon, NULL);
  } else
    printf("[WARNING] Cannot find icon for this window\n");
  ...
  for (int i = 0; i < nplayer_models; i++)
    if (player[i] == NULL)
      throw "Cannot load player 3D model";
...
void ClientRenderModule::updatePositionForPlayer(IPaddress id, int px, int py,
                                                 float *pfx, float *pfy) {
...
void ClientRenderModule::renderSingleFrame() {
  ...
  if (client_data->state != PLAYING)
    ...
    sprintf(sbuffer, "playing -> %s ( coord = %d,%d life = %d )",
            AI_state_names[client_data->purpose], px, py, life);
  font->render(rect.x + 9, rect.y + 2, sbuffer, screen);
  sprintf(sbuffer,
          "update interval: average = %.1lfms, last = %.1lfms, fps = %.1f",
          client_data->average_update_interval,
          client_data->last_update_interval, 1000.0 / frame_render_interval);
...
void ClientRenderModule::run() {
  ...
  while (client_data->state != GONE) {
    if (client_data->has_GUI) {
      ...
      stop = SDL_GetTicks();
      interval = stop - start;
      if (interval < RENDER_DELAY)
        SDL_Delay( RENDER_DELAY - interval);
