...
void init(int argc, char *argv[], ClientData &client_data) {
  ...
  if (client_data.dynamic_objects == NULL)
    throw "Not enough memory";
  for (i = 0; i < CLIENT_MATRIX_SIZE; i++) {
    client_data.dynamic_objects[i] = new char[CLIENT_MATRIX_SIZE];
    if (client_data.dynamic_objects[i] == NULL)
      throw "Not enough memory";
  ...
  if (argc < 2)
    throw "Arguments are [options] <server_name:server_port>";
  ...
  for (i = 1; i < argc - 1; i++) {
    ...
    if (!strncmp(argv[i], "--bpp=", 6)) {
      ...
      if (client_data.bpp != 1 && client_data.bpp != 8 && client_data.bpp != 16
          && client_data.bpp != 32)
  ...
  if (client_data.server_name == NULL)
    throw "Not enough memory";
  ...
  *(port_ptr++) = 0;
  ...
  if (client_data.server_port < 1)
    throw "Port number must be positive";
  printf("Connecting to server %s:%d\n", client_data.server_name,
         client_data.server_port);

  /* use DNS to get server ip address */
  SDLNet_ResolveHost(&client_data.server_address, client_data.server_name,
                     client_data.server_port);
}
...
int run_client(int argc, char *argv[]) {
  ...
  try {
    ...
    MessageModule *comm_module = new MessageModule(0, 1, 0);
    assert(comm_module);
    ...
    if (action_module == NULL)
      throw "Cannot create action module";
    SDL_Thread *action_thread = SDL_CreateThread(module_thread,
                                                 (void*) action_module);
    if (action_thread == NULL)
      throw "Cannot create action thread";

    /* RENDER */
    ClientRenderModule *render_module = new ClientRenderModule(&client_data);
    if (render_module == NULL)
      throw "Cannot create input module";
