...
void RegionWindow::OnPaint(wxPaintEvent & event) {
  ...
  for (region = 0; region < num_regions; region++) {
    ...
    dc.DrawRectangle(ri[region].x * scale, ri[region].y * scale,
                     ri[region].sizex * scale, ri[region].sizey * scale);
    dc.SetBrush(wxBrush(black));
    //TODO: some sprintf("%d",ri[region].num_players); 
    dc.DrawText(wxT("SIZE LAWL"), (ri[region].x + ri[region].sizex / 2) * scale,
                (ri[region].y + ri[region].sizey / 2) * scale);
  }
}
