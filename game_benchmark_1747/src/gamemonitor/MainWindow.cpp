...
void MainWindow::OnSave(wxCommandEvent& WXUNUSED(event)) {
  ...
  /* get file name */
  wxFileDialog dialog(this, _T("Save CSV file"), wxEmptyString,
                      _T("statistics.csv"),
  ...
  for (k = 0; k < map_data.num_threads; k++)
    fprintf(
        f,
        "Players S%d;Regions S%d;Density S%d;Crowded %d;CPU S%d;MEM S%d (MB);PMEM S%d (MB);Update interval S%d (ms);"
  ...
  for (int i = 0; i < n; i++) {
    ...
    fprintf(f, "%d;%d;%d;%d;", timestamp, number_of_players, (MAX_REGIONS),
            quest_active);
    for (k = 0; k < map_data.num_threads; k++)
      fprintf(
          f,
          "%d;%d;%.2f;%d;%d;%.2f;%.2f;%d;%.2f;%.2f;%.2f;%.2f;",
          ts[k].number_of_players,
          ts[k].number_of_regions,
          ((ts[k].number_of_regions == 0) ?
              0.0 :
              ((float) ts[k].number_of_players / (float) ts[k].number_of_regions)),
          ts[k].players_in_most_crowded_region, ss.machine_cpu_usage,
          ss.machine_mem_usage / 1024.0, ss.process_mem_usage / 1024.0,
          (int) ss.average_regular_update_interval, ss.bps_tcp_sent / 1000.0,
          ss.bps_tcp_recv / 1000.0, ss.bps_udp_sent / 1000.0,
...
void MainWindow::OnSaveReduced(wxCommandEvent& WXUNUSED(event)) {
  ...
  /* get file name */
  wxFileDialog dialog(this, _T("Save CSV reduced file"), wxEmptyString,
                      _T("statistics.csv"),
                      _T("Text CSV (*.csv)|*.csv|Text files (*.txt)|*.txt"),
                      wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
  dialog.SetFilterIndex(0);
  ...
  for (int i = 0; i < n; i++) {
    ...
    fprintf(f, "%d;%d;%d;%d;", timestamp, number_of_players, (MAX_REGIONS),
            quest_active);
    for (k = 0; k < map_data.num_threads; k++)
      fprintf(f, "%d;%d;%d;%d;%.2f;%.2f;%.2f;%.2f;", ts[k].number_of_players,
              ts[k].number_of_regions, ss.machine_cpu_usage,
              (int) ss.average_regular_update_interval,
              ss.bps_tcp_sent / 1000.0, ss.bps_tcp_recv / 1000.0,
              ss.bps_udp_sent / 1000.0, ss.bps_udp_recv / 1000.0);
    fprintf(f, "\n");
  }
...
void MainWindow::OnAbout(wxCommandEvent& WXUNUSED(event)) {
  ...
  info.SetDescription(
      _T("This application is used to monitor the game servers"));
...
wxString MainWindow::printColor(wxColor c) {
  wxString s;
  s.Printf("#%X%X%X%X%X%X", c.Red() >> 4, c.Red() & 0xF, c.Green() >> 4,
           c.Green() & 0xF, c.Blue() >> 4, c.Blue() & 0xF);
  return s;
}
...
void MainWindow::UpdateGeneral() {
  ...
  c << "<b>Partitioning algorithm: </b>" << (char*) map_data.algorithm_name
      << "<br />";
  c << "<br />";

  c << "<b>Map size: </b>" << map_data.mapx << " x " << map_data.mapy
      << "<br />";
  ...
  c << "<b>Maximum interval between quests: </b>" << map_data.quest_between
      << " seconds<br />";
  if (quest_active)
  ...
  c << "<b>Number of player migrations: </b>" << master_stats.player_migrations
      << "<br />";
  c << "<b>Number of region migrations: </b>" << master_stats.region_migrations
      << "<br />";
  c << "<br />";
...
void MainWindow::UpdateServers() {
  ...
  for (int i = 0; i < map_data.num_threads; i++) {
    ...
    c << "&nbsp;&nbsp;&nbsp;&nbsp;<b>Player density: </b>"
        << ((ts[i].number_of_regions == 0) ?
            0 : (ts[i].number_of_players / ts[i].number_of_regions))
        << "<br />";
    ...
    x = (ss.number_of_statistics == 0) ?
        0 : (ss.tcp_total / ss.number_of_statistics);
    c << "&nbsp;&nbsp;&nbsp;&nbsp;<b>Average TCP transfer: </b>"
        << printDouble(x / 1000.0) << " kb/s<br />";
    x = (ss.number_of_statistics == 0) ?
        0 : (ss.udp_total / ss.number_of_statistics);
    c << "&nbsp;&nbsp;&nbsp;&nbsp;<b>Average UDP transfer: </b>"
        << printDouble(x / 1000.0) << " kb/s<br />";
    c << "</font>";
    c << "</td>";
    if (i % server_columns == server_columns - 1)
      c << "</tr>";
  }
  c << "</table>";
  c << "</body></html>";
